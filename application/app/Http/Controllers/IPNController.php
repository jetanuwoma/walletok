...
class IPNController extends Controller
{
   ...
   public function logandpay (Request $request){
        ...
        $purchase = Purchase::create([
          ...
          'gross' =>  (float)session()->get('PurchaseRequestTotal'),
          'fee' =>  $purchase_fee,
          'net' =>  (float)session()->get('PurchaseRequestTotal') - $purchase_fee,
          'json_data' =>  json_encode($PurchaseRequest->data)
        ]);
  ...
  public function purchaseConfirmation(Request $request){
        ...
        $transaction->balance = (float)$auth_wallet->amount - (float)$transaction->net;
        $transaction->save();

        $sale_transaction->transaction_state_id = 1;
        $sale_transaction->balance =  (float) $user_wallet->amount + $sale_transaction->net;
        $sale_transaction->save();

        $auth_wallet->amount = (float)$auth_wallet->amount - (float)$transaction->net ;
        $auth_wallet->save();

        $user_wallet->amount = $user_wallet->amount + $sale_transaction->net ;
        $user_wallet->save();
